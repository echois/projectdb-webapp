...
<body>
	...
	<div id="body">
		...
		<a href="<%=request.getContextPath()%>/html/editadviser">Create
			new adviser</a>
			...
			<tbody>
				<c:forEach items="${advisers}" var="adviser">
					<tr>
						<td>&nbsp;</td>
						<td><a
							href="<%=request.getContextPath()%>/html/viewadviser?id=${adviser.id}"><img
								src="${adviser.pictureUrl}" width="60px" /></a></td>
						<td><a
							href="<%=request.getContextPath()%>/html/viewadviser?id=${adviser.id}">${adviser.fullName}</a></td>
