			...
			<tbody>
				<c:forEach items="${projectKpis}" var="projectKpi">
					<tr>
						...
						<td><nobr>${projectKpi.kpiType}-${projectKpi.kpiId}:
								${projectKpi.kpiTitle}</nobr></td>
						<td>${projectKpi.code}:${projectKpi.codeName}</td>
						<td><nobr>${projectKpi.value}</nobr></td>
						<td><nobr>${projectKpi.adviserName}</nobr></td>
						<td><a
							href="<%=request.getContextPath()%>/html/viewproject?id=${projectKpi.projectId}"><nobr>Go
									to project</nobr></a></td>
