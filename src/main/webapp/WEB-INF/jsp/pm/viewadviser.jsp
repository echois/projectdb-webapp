...
<body>
	...
	<div id="body">
		...
		<a
			href="<%=request.getContextPath()%>/html/editadviser?id=${adviser.id}">Edit</a>
		| <a
			onclick="return (confirm('Are you sure you want to delete this adviser?'))"
			href="<%=request.getContextPath()%>/html/deleteadviser?id=${adviser.id}">Delete</a>

		<h3>${adviser.fullName}</h3>

		<br>
		<img src="${adviser.pictureUrl}" width="80px" /><br>
		<br>
		...
		<br>
		<br> <b>Projects:</b><br> Total number of projects:
		${f:length(projects)}
		<p style="float: right; margin: 0">
			Get these dates as <a
				href="http://cluster.ceres.auckland.ac.nz/projects/adviser_cal.php?id=${adviser.id}">an
				ics file</a>, or import them via a <a
				href="webcal://cluster.ceres.auckland.ac.nz/projects/adviser_cal.php?id=${adviser.id}&nocache">webcal</a>
			link (Google Calendar etc)
		</p>
		<br>
		<table id="myTable" class="tablesorter">
			...
			<tbody>
				<c:forEach items="${projects}" var="project">
					<tr>
						<td><a
							href="<%=request.getContextPath()%>/html/viewproject?id=${project.id}">${project.name}</a>
