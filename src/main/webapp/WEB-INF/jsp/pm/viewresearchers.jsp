...
<body>
	...
	<div id="body">
		...
		<a href="<%=request.getContextPath()%>/html/editresearcher">Create
			new researcher</a>
			...
			<tbody>
				<c:forEach items="${researchers}" var="researcher">
					<tr>
						<td>&nbsp;</td>
						<td><a
							href="<%=request.getContextPath()%>/html/viewresearcher?id=${researcher.id}"><img
								src="${researcher.pictureUrl}" width="60px" /></a></td>
						<td><a
							href="<%=request.getContextPath()%>/html/viewresearcher?id=${researcher.id}">${researcher.fullName}</a></td>
