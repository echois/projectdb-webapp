...
<body>
	...
	<div id="body">
		...
		<a
			href="<%=request.getContextPath()%>/html/editresearcher?id=${researcher.id}">Edit</a>
		| <a
			onclick="return (confirm('Are you sure you want to delete this researcher?'))"
			href="<%=request.getContextPath()%>/html/deleteresearcher?id=${researcher.id}">Delete</a>

		<h3>${researcher.fullName}</h3>

		<br>
		<img src="${researcher.pictureUrl}" width="80px" /><br>
		<br>
		...
		<c:if test="${not empty linuxUsername}">
			...
			<br />
			<a href="${heatmapBaseUserUrl}${linuxUsername}">Currently
				Running/Queued on Auckland NeSI Cluster</a>
			<br />
			<a href="${jobauditBaseUserUrl}${linuxUsername }">Jobaudit
				Records on Auckland NeSI Cluster (History)</a>
		</c:if>
		<br>
		<br> <b>Projects:</b>
		...
		<table id="myTable" class="tablesorter">
			...
			<tbody>
				<c:forEach items="${projects}" var="project">
					<tr>
						<td><a
							href="<%=request.getContextPath()%>/html/viewproject?id=${project.id}">${project.name}</a></td>
