...
public class IBatisProjectDao extends SqlSessionDaoSupport implements
	...
	@Override
	public Integer createPerson(final Person p) throws Exception {
		getSqlSession().insert("pm.db.createPerson", p);
		return p.getId();
	}

	@Override
	public Person getPersonById(final Integer id) throws Exception {
		final Person p = (Person) getSqlSession().selectOne(
				"pm.db.getPersonById", id);
		p.setNumProjects(getNumProjectsForAdviser(p.getId()));
		return p;
	}

	@Override
	public List<Person> getPersons() throws Exception {
		final List<Person> list = getSqlSession()
				.selectList("pm.db.getPersons");
		for (final Person p : list) {

			// TODO: Update to getNumProjectsForPerson
			p.setNumProjects(getNumProjectsForAdviser(p.getId()));
		}
		return list;
	}
