...
public class ResearcherControls extends AbstractControl {
	...
	public static void main(String[] args) throws Exception {
		...
		ResearcherControls rc = (ResearcherControls) context
	 ...
	 */
	public Integer createResearcher(Researcher researcher)
			throws InvalidEntityException {
	 ...
	 * @throws InvalidEntityException
	 */
	public void editResearcher(Integer id, String field, String timestamp,
			String data) throws OutOfDateException, NoSuchEntityException,
	...
	public List<Researcher> filterResearchers(String filter) {
		...
		for (Researcher r : this.getAllResearchers()) {
			if (r.getAffiliation().toLowerCase().contains(filter)
					...
					|| r.getStatusName().toLowerCase().contains(filter)) filtered
					.add(r);
		}
	...
	private void validateResearcher(Researcher r) throws InvalidEntityException {
		...
		if (r.getPhone().trim() != "" && !r.getPhone().matches(".+[0-9].+")) throw new InvalidEntityException(
				"Phone must contain at least one digit", Researcher.class,
				"phone");
		if (r.getEmail().trim() != "" && !r.getEmail().matches(".+@.+[.].+")) throw new InvalidEntityException(
				"Not a valid email", Researcher.class, "email");
