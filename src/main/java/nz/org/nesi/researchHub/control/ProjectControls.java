...
public class ProjectControls extends AbstractControl {
	...
	public Map<String, Map<String, Set<String>>> getAllProjectsAndMembers()
		...
		Map<String, Map<String, Set<String>>> allProjects = projectDao
				.getAllProjectsAndMembers();
	 ...
	 * 
	 */
	public void upsertProjectAllocation(ProjectAllocation pa) throws Exception {
		try {
			if (pa.getId() != null) {
				ProjectAllocation old = projectDao.getProjectAllocationById(pa
						.getId());
				projectDao.upsertProjectAllocation(pa);
				if (!old.getFacility().toLowerCase()
						.equals(pa.getFacility().toLowerCase())) {
					Change ch = new Change();
					ch.setTbl_id(pa.getId());
					ch.setTbl("projectallocation");
					ch.setField("facility");
					ch.setAdviserId(this.authzAspect.getAdviserId());
					ch.setNew_val(pa.getFacility());
					ch.setOld_val(old.getFacility());
					projectDao.logChange(ch);
				}
				if (!old.getProjectCode().toLowerCase()
						.equals(pa.getProjectCode().toLowerCase())) {
					Change ch = new Change();
					ch.setTbl_id(pa.getId());
					ch.setTbl("projectallocation");
					ch.setField("project");
					ch.setAdviserId(this.authzAspect.getAdviserId());
					ch.setNew_val(pa.getProject());
					ch.setOld_val(old.getProject());
					projectDao.logChange(ch);
				}
				if (!old.getAllocation().equals(pa.getAllocation())) {
					Change ch = new Change();
					ch.setTbl_id(pa.getId());
					ch.setTbl("projectallocation");
					ch.setField("allocation");
					ch.setAdviserId(this.authzAspect.getAdviserId());
					ch.setNew_val(pa.getAllocation());
					ch.setOld_val(old.getAllocation());
					projectDao.logChange(ch);
				}
			} else {
				projectDao.upsertProjectAllocation(pa);
